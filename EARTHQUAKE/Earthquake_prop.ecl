...
export earthquake_prop := MODULE
import $;
earthquakes := $.BWRearthquake.earthquakes;

maxhomeless := max(earthquakes, homeless);
maxinjured := max(earthquakes,injured);
maxaffected := max(earthquakes,affected);
maxdamages := max(earthquakes,damages);
maxtotaldeath := max(earthquakes,totaldeath);

myrec16 := RECORD
    recordof(earthquakes);
    decimal RelHomeless;
    decimal RelInjured;
    decimal RelAffected;
    decimal RelDamages;
...
myrec16 T6(earthquakes l) := TRANSFORM
    SELF := l;
...
export mytable16 := PROJECT(earthquakes,T6(left) );
//mytable16;
