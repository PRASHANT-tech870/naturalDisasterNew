...
export Drought_prop := MOdule 
import $;
droughts := $.BWRDroughts.Droughts;

maxhomeless := max(droughts, homeless);
maxinjured := max(droughts,injured);
maxaffected := max(droughts,affected);
maxdamages := max(droughts,damages);
maxtotaldeath := max(droughts,totaldeath);
...
myrec16 T6(droughts l) := TRANSFORM
    SELF := l;
...
export mytable16 := PROJECT(droughts,T6(left) );
// mytable16;
...
// AddSeverity_Score;
// export AddSeverity_Score,'~class::RDT::INTRO::CrimeScore',NAMED('TopCrime'),OVERWRITE;
