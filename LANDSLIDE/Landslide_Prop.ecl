...
export landslide_prop := module
import $;
landslides := $.BWRlandslides.landslides;

maxhomeless := max(landslides, homeless);
maxinjured := max(landslides,injured);
maxaffected := max(landslides,affected);
maxdamages := max(landslides,damages);
maxtotaldeath := max(landslides,totaldeath);
...
myrec16 T6(landslides l) := TRANSFORM
    SELF := l;
...
export mytable16 := PROJECT(landslides,T6(left) );
// mytable16;
